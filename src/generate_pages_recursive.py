...
def generate_pages_recursive(base_path, from_path, template_path, dest_path):
    ...
    generate_pages(base_path, from_path, template_path, dest_path)

def generate_pages(base_path, from_path, template_path, dest_path):
    for file in os.listdir(from_path):
        ...
        if is_dir:
            ...
            generate_pages(base_path, file_path_source, template_path, file_path_target)
        if is_file and file.endswith(".md"):
            dest_filename = file[:-3] + ".html"
            file_path_target = os.path.join(dest_path, dest_filename)
            generate_page(base_path, file_path_source, template_path, file_path_target)